...
def create_customer_app(root,font):
...
#
   ...
   lbl_del_customer.grid(row=14, column=1)
   entry_del_customer.grid(row=15, column=0)
   btn_del_customer.grid(row=15, column=1)
...
def fun_new_customer(first_name,last_name,email,phone_number):
   data = f'{{ "customername": "{first_name}", "password": "{last_name}", "card": "{email}", "phone_number": "{phone_number}"}}'
